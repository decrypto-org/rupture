...
SameSite cookies \cite{c8} is a proposed draft for enhancing web security. This policy
...
implementing this class of attacks called Rupture \cite{c11}. Rupture was
...
parameterized with an alphabet of ASCII symbols $a$ for each character of the
...
attack is $\mathcal{O}(|a|)$ and the round ends by finding a character of the
secret.

The second method of attack is divide and conquer. In each phase the alphabet is
divided into two subsets $a_1$ and $a_2$, where $|a_1| = |a_2| = \ceil{|a| /
2}$. The reflection string for $a_1$ composes of $|a_1|$ substrings divided by
an annotation symbol $\beta$. Each substring consists of the known part of the
secret concatenated with a character in $a_1$. The reflection string for $a_2$
is constructed similarly. Each phase consists of one call to the reflection
oracle per subset. As long as the prefix of the secret is
\textit{compression-detectable} by each substring of the reflection, the end of
each phase marks the choice of subset $a_i$ that contains the correct alphabet
symbol, in which case $a = a_i$. The round is complete when $|a| = 1$. The
complexity of the attack in this case is $\mathcal{O}(lg(|a|))$.
