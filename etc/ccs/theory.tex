...
The distribution $\mathcal{M}$ is ${p_s:t}$, where $t \in \mathcal{A}$ and
$\mathcal{A}$ is a known alphabet of ASCII symbols.
...
issue requests on different elements of $A$ and compare the response lengths. As
...
Our contributions include the development of a production-grade framework,
Rupture \cite{c11}, for implementing this class of attacks. Rupture was
developed as part of the work on extending BREACH application against modern
systems and block ciphers \cite{c12}.
...
In addition, Rupture offers multiple instances of oracle $\mathcal{O}_R$ and
enables the automated computation of the reflection strings in each stage of the
attack.

Finally, it amplifies the attack by issuing multiple requests per candidate
symbol in alphabet $\mathcal{A}$ in order to demonstrate better probability of
success.

\subsection{Rupture architecture}\label{app:rupture}
Rupture consists of multiple components that collaborate in executing
compression-based attacks, such as BREACH.

The client component is a Javascript code that issues requests towards a chosen
endpoint. Client needs to be executed from the browser of the victim that the
adversary is trying to steal secrets from.

The injector component injects the client code in the plaintext of victim's
HTTP responses. In order for the injection to be performed, the adversary should
be able to view and alter the victim's network traffic.

The sniffer component also needs access to the victim's network traffic. As long
as this is possible, the sniffer keeps track of the encrypted communication
between the victim and the endpoint and reports the packet lengths observer on
the network.

The backend component is a Python code that controls the attack execution. It
initializes the attack, guides the client as to what requests need to be made at
each stage of the attack and orders sniffer to listen and report network
traffic. At each stage of the attack, the backend analyzes the sniffer reports
and calculates the probability of success for the attack. At the end of each
stage, a single character of the secret is stolen and the backend uses this
finding to continue the attack against the rest of the secret.
...
\subsection{Reflection computation methods}
...
parameterized with an alphabet of ASCII symbols $\mathcal{A}$ for each character of the
...
when requests have been issued for the entire alphabet. The complexity of the
attack is $\mathcal{O}(|\mathcal{A}|)$ and the round ends by finding a character of the
secret.

The second method of attack is divide and conquer. In each phase the alphabet is
divided into two subsets $\mathcal{A}_1$ and $\mathcal{A}_2$, where
$|\mathcal{A}_1| = |\mathcal{A}_2| = \ceil{|\mathcal{A}| / 2}$. The reflection
string for $\mathcal{A}_1$ composes of $|\mathcal{A}_1|$ substrings divided by
an annotation symbol $\beta$. Each substring consists of the known part of the
secret concatenated with a character in $\mathcal{A}_1$. The reflection string
for $\mathcal{A}a_2$ is constructed similarly. Each phase consists of one call
to the reflection oracle per subset. As long as the prefix of the secret is
\textit{compression-detectable} by each substring of the reflection, the end
of each phase marks the choice of subset $\mathcal{A}_i$ that contains the
correct alphabet symbol, in which case $\mathcal{A} = \mathcal{A}_i$. The
round is complete when $|\mathcal{A}| = 1$. The complexity of the attack in
this case is $\mathcal{O}(lg(|\mathcal{A}|))$.

\subsection{Request parallelization}
Reflection oracle is generally considered as synchronous. However, this is not
...
Modern web servers are able to handle multiple requests per millisecond and
browsers offer a certain amount of parallel requests per domain. This
functionality enables the adversary to issue multiple parallel requests per
symbol in alphabet $\mathcal{A}$ and efficiently reduce the execution time of
the attack.
