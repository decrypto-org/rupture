...
such as CRIME, TIME, and BREACH. We introduce an abstract theoretical model for
such attacks, the \textit{adaptive reflection security} model. We discuss what
properties common compression functions have, such as the
...
functions. For the case of the BREACH attack, we show that it can be described
as an instance of our theoretical model. Motivated by the lack of
production-grade testing tools, we provide a modular scalable open-source
generic attack framework that implements these attacks in a robust manner.
Furthermore, we explore the performance of defenses described in literature and
propose a novel protocol that effectively eliminates the threat these attacks
pose.
\end{abstract}
...
Let $\mathcal{S}, \mathcal{R}$ be two distributions of random values $s$, $r$
and $\bar{M}$ their joint distribution. We say that $s$ and $r$ are
interdependent when:
