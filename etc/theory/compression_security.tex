...
when the alphabet is the nuber digits. In order to find the correct leaf of the
...
the target service. Injection in Rupture is achieved through the
...
client from the \textit{realtime} component to which the client maintains a
persistent connection.

The real-time component is only responsible for communicating with the client.
The real decisions for the attack are driven by the \textit{backend}, which
maintains a persistent attack state. The back-end stores persistent data in a
SQLite \textit{database} and receives data from the sniffer to perform analyses.
